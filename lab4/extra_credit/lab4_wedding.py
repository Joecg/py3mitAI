...
def forward_checking_prop_singleton(state, verbosity = 0):
    ...
    if verbosity > 4: print('Forward checking without propagation'+
                            ' complete.')
    table_fillings = dict()
    queue = []
    for v in state.get_all_variables():
        if v.is_assigned():
            table = v.get_assigned_value()
            if table in table_fillings:
                if len(table_fillings[table]) == 10: return False
                table_fillings[table].append(v)
            else: table_fillings[table] = [v]
        elif v.domain_size() == 1:
            queue.append(v)
            table = v.get_domain()[0]
            if table in table_fillings:
                if len(table_fillings[table]) == 10: return False
                table_fillings[table].append(v)
            else: table_fillings[table] = [v]
        ...
        for c in X_constraints:
            ...
            if type(c) == BinaryConstraint:
                ...
                if ((Y.domain_size() == 1) and
                    ...
                    not (Y in visited)):
                    queue.append(Y)
                    table = Y.get_domain()[0]
                    if table in table_fillings:
                        if len(table_fillings[table]) == 10: return False
                        table_fillings[table].append(Y)
                    else: table_fillings[table] = [Y]
            elif type(c) == GroupConstraint:
