...
def show_result(testsummary, testcode, correct, got, expected, args, verbosity):
    if correct:
        if verbosity > 0:
...
def show_exception(testsummary, testcode, args):
    print("%s: Error." % testsummary)
    print("While running the following test case:")
    print('\t', testcode)
    print("Which included arguments:")
    print('\t', args)
...
def test_offline(verbosity=1):
    ...
    for index, (testname, getargs, testanswer, expected, fn_name) in enumerate(tests):
        ...
        try:
            ...
            show_exception(summary, testname, getargs())
            continue
        
        show_result(summary, testname, correct, answer, expected, getargs(), verbosity)
